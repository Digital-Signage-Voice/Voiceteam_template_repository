...
# [2단계] 설정
# =========================================================
VIDEO_PATH = "C:/shinhan/DigitalSignage/data/video/IMG_4635.MOV"
...
def run_audio_pipeline(audio_data, config, vad_history, mode='standard'):
    ...
    print(f"   ℹ️ [Quality] 고품질 오디오 생성 중 ({mode})...")
...
# =========================================================
# [New] STT 수행 함수 (필터링 기능 추가)
# =========================================================
def perform_stt(model, audio_data, case_name):
    print(f"📝 [STT] {case_name} 변환 중...")
    
    # 1. 오디오 에너지가 너무 낮으면(완벽한 침묵) STT 스킵 (Hallucination 방지 핵심)
    ...
    try:
        segments, info = model.transcribe(
            ...
            condition_on_previous_text=False # 이전 텍스트 맥락 끊기 (반복 방지)
        )
        
        text_segments = []
        for s in segments:
            # 2. 반복 필터링 (동일 문구가 연속되면 제외)
            if text_segments and s.text.strip() == text_segments[-1]:
                continue
            text_segments.append(s.text.strip())
            
        full_text = " ".join(text_segments).strip()
        
        # 3. 특정 환각 키워드 제거 (Whisper 고질병)
...
# =========================================================
def run_benchmark():
    print(f"\n🚀 [Benchmark] Final Logic (Clicking Free + STT Save)")
    ...
    # --- Case 3: Proposed ---
    print("\n3️⃣ [Case 3] Proposed")
    # [수정된 파라미터 적용: VAD 민감도 0.2, 감쇠 0.1]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "benchmark_chart_final.png"))
    print("✅ 모든 작업 완료.")
